...
// given path.
func (n *node) getValue(path string, params func() *Params) (apiID uint64, ps *Params, tsr bool) {
	...
	for {
		...
		if len(path) > len(prefix) {
			if path[:len(prefix)] == prefix {
				...
				case param:
					...
					if apiID = n.apiID; apiID != 0 {
						return
					} else if len(n.children) == 1 {
						// No handle found. Check if a handle for this path + a
						// trailing slash exists for TSR recommendation
						n = n.children[0]
						tsr = n.path == "/" && n.apiID != 0
					}
			...
			for i, c := range []byte(n.indices) {
				if c == '/' {
					n = n.children[i]
					tsr = (len(n.path) == 1 && n.apiID != 0) ||
						(n.nType == catchAll && n.children[0].apiID != 0)
					return
				}
			}
			return
		...
		tsr = (path == "/") ||
			(len(prefix) == len(path)+1 && prefix[len(path)] == '/' &&
				path == prefix[:len(prefix)-1] && n.apiID != 0)
		return
	}
}
...
walk: // Outer loop for walking the tree
	for len(path) >= npLen && (npLen == 0 || strings.EqualFold(path[1:npLen], n.path[1:])) {
		...
		if len(path) > 0 {
			...
			if !n.wildChild {
				...
				if fixTrailingSlash && path == "/" && n.apiID != 0 {
					return ciPath
				}
				return nil
			}
	...
	if fixTrailingSlash {
		...
		if len(path)+1 == npLen && n.path[len(path)] == '/' &&
			strings.EqualFold(path[1:], n.path[1:len(path)]) && n.apiID != 0 {
